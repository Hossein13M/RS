...
import { Tree, TreeFlatNode, TreeNode } from '../tree-select/tree-select.types';
// import { Tree, TreeFlatNode, TreeNode } from './tree-select.types';
@Component({
  selector: 'tree-single-select',
  templateUrl: './tree-single-select.component.html',
  styleUrls: ['./tree-single-select.component.scss'],
  providers: [
    { provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => TreeSingleSelectComponent), multi: true },
    { provide: NG_VALIDATORS, useExisting: forwardRef(() => TreeSingleSelectComponent), multi: true },
],
animations: fuseAnimations,
})
export class TreeSingleSelectComponent {
    ...
    isZerolevel=(_: number, nodeData: TreeFlatNode)=>nodeData.level==0;
